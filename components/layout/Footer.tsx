...
const QUICK_LINKS = [
  { href: "/", label: "Accueil" },
  { href: "/qui-est-dr-yasso", label: "Qui est Dr YASSO ?" },
    ...
    <footer className="bg-[#081D3E] text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div className="lg:col-span-1">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-12 h-12 border-2 border-[#C89B3C] flex items-center justify-center">
              <span className="text-[#C89B3C] font-display font-bold text-xl">DY</span>
            </div>
            <div>
          ...
          <p className="text-white/50 font-body text-sm leading-relaxed">
            Engagé pour le développement de la 4e circonscription électorale — Natitingou et Toucountouna.
          </p>
          {/* Social */}
        ...
        <div>
          ...
          <ul className="space-y-2 text-white/60 font-body text-sm">
            <li className="flex items-center gap-2">
              <span className="text-[#6EC6FF]">◆</span> Natitingou (Chef-lieu)
            </li>
            <li className="flex items-center gap-2">
              <span className="text-[#6EC6FF]">◆</span> Toucountouna
            </li>
            <li className="flex items-center gap-2">
              <span className="text-[#6EC6FF]">◆</span> Villages périphériques
            </li>
            <li className="flex items-center gap-2">
              <span className="text-[#6EC6FF]">◆</span> Région de l&apos;Atacora
            </li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-4 text-white/60 font-body text-sm">
            <li>
              <span className="block text-white/30 text-xs mb-1 uppercase tracking-wider">Adresse</span>
              Natitingou, Atacora, Bénin
            </li>
            <li>
              <span className="block text-white/30 text-xs mb-1 uppercase tracking-wider">Email</span>
              <a href={`mailto:${SITE_CONFIG.email}`} className="hover:text-[#C89B3C] transition-colors">
                {SITE_CONFIG.email}
              </a>
            </li>
            <li>
              <span className="block text-white/30 text-xs mb-1 uppercase tracking-wider">Téléphone</span>
              {SITE_CONFIG.phone}
            </li>
          </ul>
        </div>
      ...
      <div className="border-t border-white/10 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-white/30 font-body text-xs">
          <p>© {new Date().getFullYear()} Dr Désiré YASSO — Tous droits réservés.</p>
          <p className="text-center">
            4e Circonscription électorale · Natitingou & Toucountouna · Atacora, Bénin
          </p>
        </div>
      </div>
    </footer>
