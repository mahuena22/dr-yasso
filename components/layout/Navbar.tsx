        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="w-10 h-10 border-2 border-[#C89B3C] flex items-center justify-center">
              <span className="text-[#C89B3C] font-display font-bold text-lg leading-none">DY</span>
            </div>
            <div className="hidden sm:block">
              <div className="text-white font-display font-bold text-base leading-tight tracking-wide">
                Dr Désiré YASSO
              </div>
              <div className="text-[#C89B3C] font-body text-[10px] tracking-[0.2em] uppercase">
                4e Circonscription · Atacora
              </div>
            </div>
          </Link>
            ...
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className={`relative px-4 py-2 font-body text-sm font-500 tracking-wide transition-colors duration-200 group ${
                  isActive(link.href) ? "text-[#C89B3C]" : "text-white/80 hover:text-white"
                }`}
              >
                {link.label}
                <span
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              aria-label="Menu"
            >
              <span className={`hamburger-line text-white ${menuOpen ? "line-1" : ""}`} />
              <span className={`hamburger-line text-white ${menuOpen ? "line-2" : ""}`} />
              <span className={`hamburger-line text-white ${menuOpen ? "line-3" : ""}`} />
            </button>
          </div>
        </div>
      </header>
          ...
          {NAV_LINKS.map((link, i) => (
            <Link
              ...
              className={`font-display text-3xl font-700 py-3 border-b border-white/10 transition-all duration-300 ${
                isActive(link.href) ? "text-[#C89B3C]" : "text-white/70 hover:text-white"
              }`}
